...
Мій внесок — два підмодулі, що є «фундаментом» системи: керування доступом і клієнти.
На них спираються інші командні модулі (замовлення, резервації, персонал).
...
> має складений первинний ключ із двох зовнішніх ключів. 
...
### 5. `customers` — клієнти
**Спільна командна таблиця.** На неї посилаються замовлення та резервації інших модулів.
...
*(Final-версія вимагає «explanations of design choices».)*

- **Ролі ≠ посади.** `roles` — це ролі *доступу* (права в застосунку), а не посади
  працівників. Посади живуть у таблиці `staff` (модуль «Персонал»). Офіціант (посада)
  може мати роль доступу `cashier`.
